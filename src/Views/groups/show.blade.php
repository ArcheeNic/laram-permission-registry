    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <div class="mb-8">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('permission-registry::Group Information') }}</h3>
                    <div class="bg-gray-50 p-4 rounded-md">
                        <dl class="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
                            ...
                            <div>
                                <dt class="text-sm font-medium text-gray-500">{{ __('permission-registry::Created At') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $group->created_at->format('d.m.Y H:i') }}</dd>
                            </div>
                ...
                <div>
                    <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('permission-registry::Permissions in Group') }}</h3>
                    @if($group->permissions->isEmpty())
                        <p class="text-sm text-gray-500">{{ __('permission-registry::No permissions assigned to this group') }}</p>
                    @else
