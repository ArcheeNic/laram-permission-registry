    ...
    <div class="bg-white dark:bg-neutral-800 rounded-lg shadow">
        <div class="p-4 border-b dark:border-gray-700">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                {{ __('permission-registry::governance.attestations') }}
        ...
        @if($attestations->isEmpty())
            <div class="p-8 text-center text-gray-500 dark:text-gray-400">
                {{ __('permission-registry::governance.no_attestations') }}
            </div>
        @else
            {{-- Desktop table --}}
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-neutral-700">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase hidden lg:table-cell">{{ __('permission-registry::governance.decided_by') }}</th>
                        ...
                        @foreach($attestations as $attestation)
                            <tr class="hover:bg-gray-50 dark:hover:bg-neutral-700/50">
                                <td class="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">
                                    {{ $attestation->grantedPermission->permission->service ?? '—' }} / {{ $attestation->grantedPermission->permission->name ?? '—' }}
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                                    {{ $attestation->grantedPermission->user->name ?? '—' }}
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    @php
                                        $attStatusClasses = match($attestation->status->value) {
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $attStatusClasses }}">
                                        {{ $attestation->status->label() }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 hidden lg:table-cell">
                                    {{ $attestation->due_at?->format('d.m.Y H:i') ?? '—' }}
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 hidden lg:table-cell">
                                    {{ $attestation->decider->name ?? '—' }}
                                </td>
                                <td class="px-4 py-3 text-right">
                                    @if(!$attestation->status->isTerminal())
                                        <div class="flex justify-end gap-2">
                                            <button wire:click="openDecisionModal({{ $attestation->id }}, 'confirmed')"
                                                ...
                                                {{ __('permission-registry::governance.confirm_attestation') }}
                                            </button>
                                            <button wire:click="openDecisionModal({{ $attestation->id }}, 'rejected')"
                                                    class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-md transition-colors
                                                           bg-red-100 text-red-700 hover:bg-red-200
                                                           dark:bg-red-900 dark:text-red-300 dark:hover:bg-red-800">
                                                {{ __('permission-registry::governance.reject_attestation') }}
                                            </button>
                                        </div>
                                    @endif
                                </td>
                ...
                @foreach($attestations as $attestation)
                    <div class="p-4 space-y-2">
                        ...
                        <div class="text-xs text-gray-500 dark:text-gray-400">
                            {{ __('permission-registry::governance.due_at') }}: {{ $attestation->due_at?->format('d.m.Y') ?? '—' }}
                        </div>
                        @if(!$attestation->status->isTerminal())
                            <div class="flex justify-end gap-2">
                                <button wire:click="openDecisionModal({{ $attestation->id }}, 'confirmed')"
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white dark:bg-neutral-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="px-4 pt-5 pb-4 sm:p-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                {{ __('permission-registry::messages.comment') }}
                                <x-perm::field-hint
                                    :title="__('permission-registry::hints.attestations_comment_title')"
                                    :description="__('permission-registry::hints.attestations_comment_desc')"
                                />
                            </label>
                            <textarea wire:model="comment" rows="3"
                                      class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-neutral-700 dark:text-gray-200 shadow-sm text-sm"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 dark:bg-neutral-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                        ...
                        @else
                            <button wire:click="submitDecision"
                                    class="w-full sm:w-auto px-4 py-2 bg-red-600 text-white text-sm rounded hover:bg-red-700 dark:bg-red-500 dark:hover:bg-red-600 transition">
                                {{ __('permission-registry::governance.reject_attestation') }}
                            </button>
                        @endif
