...
<div class="p-4 border-b dark:border-neutral-700 flex flex-wrap gap-2 items-center">
    <button
        ...
        class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-neutral-700 dark:text-gray-300 dark:hover:bg-neutral-600
               {{ empty($statusFilterValues) ? 'ring-2 ring-indigo-500 ring-offset-1 dark:ring-offset-neutral-800' : '' }}">
        {{ __('permission-registry::messages.import.filter_all') }}
        <span class="ml-1 font-bold">{{ $stagingStats['total'] }}</span>
    </button>
    @foreach($filters as $value => $filter)
        <button
            ...
            class="px-3 py-1.5 text-xs font-medium rounded-md transition-colors {{ $filter['classes'] }}
                   {{ in_array($value, $statusFilterValues, true) ? 'ring-2 ring-indigo-500 ring-offset-1 dark:ring-offset-neutral-800' : '' }}">
            {{ $filter['label'] }}
