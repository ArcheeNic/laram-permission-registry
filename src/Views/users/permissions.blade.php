            ...
            <div class="bg-white dark:bg-neutral-800 overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <div class="mb-6">
                    ...
                    @if($grantedPermissions->isEmpty())
                        <p class="text-gray-500 dark:text-gray-400">{{ __('permission-registry::Права еще не выданы') }}</p>
                    @else
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <form action="{{ route('permission-registry::users.permissions.revoke', [$user->id, $grantedPermission->id]) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 dark:text-red-300 hover:text-red-900" onclick="return confirm('{{ __('permission-registry::Вы уверены, что хотите отозвать это право?') }}')">
                ...
                <div class="mt-10">
                    ...
                    <form action="{{ route('permission-registry::users.permissions.grant', $user->id) }}" method="POST" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="permission_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    {{ __('permission-registry::Право доступа') }}*
                                    <x-perm::field-hint
                                        :title="__('permission-registry::hints.users_permissions_permission_title')"
                                        :description="__('permission-registry::hints.users_permissions_permission_desc')"
                                    />
                                </label>
                                <select id="permission_id" name="permission_id" required
                                        class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 dark:border-neutral-600 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                            ...
                            <div>
                                <label for="expires_at" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                    ...
                                    <x-perm::field-hint
                                        :title="__('permission-registry::hints.users_permissions_expires_at_title')"
                                        :description="__('permission-registry::hints.users_permissions_expires_at_desc')"
                                    />
                                </label>
