...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Общие и служебные ящики не принадлежат конкретному сотруднику, а
    | сопоставление строк импорта с реестром идёт именно по почте. Адреса из
    | этого списка не сопоставляются и не попадают в кандидаты на отзыв.
    |
    */
    'import' => [
        'excluded_emails' => array_values(array_filter(array_map(
            static fn (string $email): string => mb_strtolower(trim($email)),
            explode(',', (string) env('IMPORT_EXCLUDED_EMAILS', ''))
